...
export function localVpCommand(repoRoot: string, args: ReadonlyArray<string>) {
  return {
...
export const MAC_ENTITLEMENTS = `<?xml version="1.0" encoding="UTF-8"?>
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  if (!options.skipBuild) {
    ...
    const spawnCommand = localVpCommand(repoRoot, ["run", "build:desktop"]);
    yield* runCommand(
      ChildProcess.make(spawnCommand.command, spawnCommand.args, {
        cwd: repoRoot,
        shell: spawnCommand.shell,
  ...
  // Keep the upstream copyright and permission notice in every desktop artifact.
  ...
  if (macEntitlementsPath) {
    yield* fs.writeFileString(macEntitlementsPath, MAC_ENTITLEMENTS);
  }
  ...
  const installCommand = localVpCommand(repoRoot, [...STAGE_INSTALL_ARGS]);
  yield* runCommand(
  ...
  const builderCommand = localVpCommand(repoRoot, builderArgs);
  yield* runCommand(
