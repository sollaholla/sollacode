...
export function buildVoiceTranscriptCorrectionPrompt(input: VoiceTranscriptCorrectionPromptInput) {
  ...
  const prompt = [
    ...
    "- after the grammar pass, check each clause for plausible-looking homophones, sound-alikes, or clipped words that make it semantically incoherent; correct them when one phonetically similar reading is strongly implied by the sentence or conversation context",
    "- preserve intentional fragments and informal speech; remove only accidental false starts, filler, or repetition",
    "- preserve the user's wording instead of making it more formal, concise, or polished beyond the corrections needed for an accurate transcript",
    ...
    "- keep uncertainty unchanged only when multiple coherent readings remain; do not preserve an incoherent phrase when one phonetically similar reading clearly fits",
    ...
    "- Raw: Show the transcription in a short form with lip but then when hover over it expands fully",
    "  Corrected: Show the transcription in a short form with an ellipsis, but then, when you hover over it, it expands fully.",
    "- Raw: Use a cup with a lip so it does not spill",
    "  Corrected: Use a cup with a lip so it does not spill.",
    "",
