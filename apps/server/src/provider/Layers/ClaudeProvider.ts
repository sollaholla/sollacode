...
/**
 * Capabilities for a hand-typed Claude model id. Every current Claude model
 * takes `--effort`, so a custom id gets the full reasoning picker rather than
 * no options. No context-window option: the `[1m]` suffix is only known to
 * exist for catalogued models, and without a selection the adapter keeps
 * trusting the window the SDK reports for the model.
 */
    ...
    capabilities: createModelCapabilities({
      optionDescriptors: [
        ...
        buildSelectOptionDescriptor({
          ...
          // Claude Code selects the 1M variant explicitly (`claude-fable-5-1[1m]`).
          options: [
...
export function normalizeClaudeCliEffort(
  ...
  // The downgrades below encode what each catalogued model accepts; a custom
  // id is the user's own claim about the model, so honour its effort verbatim.
  if (!BUILT_IN_MODELS.some((candidate) => candidate.slug === model)) {
    return effort;
  }
  if (
    effort === "xhigh" &&
