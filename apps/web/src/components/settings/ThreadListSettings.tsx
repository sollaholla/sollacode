...
/**
 * Thread-list presentation settings.
 *
 * Graduated out of the Beta section once the flat list and auto-settle had
 * been running as the default long enough to be ordinary behaviour.
 */
export function ThreadListSettingsSection() {
  ...
  return (
    <SettingsSection title="Thread list">
      <SettingsRow
        title="Flat thread list"
        description="One flat list in creation order. Active work renders as rich cards; settled threads collapse to compact rows. Settling requires an up-to-date server — on older servers threads simply stay active. Switch back any time."
        control={
          <Switch
            checked={sidebarV2Enabled}
            // Touching the switch pins the choice, so a nightly build that
            // defaults it on does not flip it back after the user opts out.
            ...
            aria-label="Use the flat thread list"
          />
        }
      />
      {sidebarV2Enabled ? (
