...
/**
 * Derived from the contract rather than hand-listed. The previous inline
 * `=== "plan" || === "default"` checks silently rejected "agent" when that mode
 * was added, so selecting Agent fell straight back to Build.
 */
const isInteractionMode = Schema.is(ProviderInteractionMode);
  ...
  if (rawDraftThreadsByThreadId && typeof rawDraftThreadsByThreadId === "object") {
    for (const [threadKeyOrId, rawDraftThread] of Object.entries(
      ...
      draftThreadsByThreadKey[threadKey] = {
        ...
        interactionMode: isInteractionMode(candidateDraftThread.interactionMode)
...
function normalizePersistedDraftsByThreadId(
  ...
  for (const [threadKeyOrId, draftValue] of Object.entries(
    ...
    const runtimeMode = isRuntimeMode(draftCandidate.runtimeMode)
      ...
      : null;
    const interactionMode = isInteractionMode(draftCandidate.interactionMode)
    ...
    (setBase, get) => {
      ...
      return {
        ...
        setInteractionMode: (threadRef, interactionMode) => {
          ...
          const nextInteractionMode = isInteractionMode(interactionMode) ? interactionMode : null;
          set((state) => {
            const existing = state.draftsByThreadKey[threadKey];
