...
This guide is for people who want to use more than one Codex account in Solla Code.
...
In Solla Code Settings, name it something obvious:
...
In Solla Code Settings, add another Codex provider:
...
Solla Code shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Solla Code treats it
