...
Solla Code works with the platforms your team already uses:
...
- Solla Code can suggest titles and descriptions based on your commits
...
1. Install the GitHub CLI on the machine running Solla Code:
   ```bash
   brew install gh
   ```
2. Sign in:
...
2. Add these environment variables to the environment running Solla Code:
   ```bash
   export T3CODE_BITBUCKET_EMAIL="you@example.com"
   export T3CODE_BITBUCKET_API_TOKEN="your-token"
   ```
3. Restart Solla Code and verify the connection in **Source Control settings**
...
**Git is required** – Solla Code uses Git for all local operations. Ensure `git` is installed on your server.

**Server-side setup** – Authentication happens on the machine running Solla Code (the server), not your local browser. If you're using a hosted or team instance, your administrator may have already configured providers.
